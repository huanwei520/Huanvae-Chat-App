...
> 🔒 **本文件在 PUBLIC 仓**：远程构建宿主的**地址 / 账号 / 密钥文件名一律不写在这里**，
> 一律以「远程 Android 构建宿主」指代；连接信息见**工作区内部记录**（非本仓）：
> `work/.claude/CLAUDE.md` 与监督者记忆 `aihost-android-build-mobile-screenshots`。
> 修改本文件时若要补充操作细节，**先问一句「这条写进公开仓合不合适」**。

> 出处：2026-08-11 用户明令 —— 「(远程编译服务器)不是有用来做过吗，当时不是已经截过图了，
...
## 二、正路：远程 Android 构建宿主 + KVM 模拟器 + adb
...
- 地址 / 账号：**不写在本仓**（PUBLIC）。见工作区内部记录（`work/.claude/CLAUDE.md`、监督者记忆）。
  ⚠️ 该宿主有两个网络路径，从开发 VM 只有其中一条通 —— 用错那条会连不上，取地址时一并看清楚。
- 免密：已配置统一 SSH key（**文件名不写在本仓**，见内部记录）
- 2026-07-25 实测：x86_64 Linux，**`/dev/kvm` 在**（硬件 KVM，模拟器很快），资源充裕
...
1. **同步源码**：从开发机的 `Huanvae-Chat-App` 工作树 `rsync` 到该构建宿主
...
**「拿不到」的合法形态**：写明尝试到第几步、失败原因（如「构建宿主上 AVD 不存在需重建」「/dev/kvm 不可用」），
...
不变的是三条：① 本机硬件级跑不了移动模拟器；② 远程构建宿主是那条路径的载体；
