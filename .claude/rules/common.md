...
（同族：`common.md` 顶部「`grep` 是 shell-snapshot 里的 ugrep 包装函数」—— 都是**工具层**把结论改了。）
...
`?head_sha=<负向>` → `total_count=0` + `workflow_runs: []`。负对照（假 sha / 假 run id）应给 **422 / 404**，
与 200 形状不同 ⇒ 那些 200 是真 200。
...
> 本次实测：正向 push 完成 → run 被创建**仅 2 秒**，首次查询（+15s）即 `total_count=1`；
> 负向按红线等满 120s 后仍为 `0` / `workflow_runs: []`。两条记录的原始 JSON 与假 sha 的 422
> 落在 gen-22 证据目录；**修好后照出的存量红结论以那次实跑为准**（本节不预设）。
