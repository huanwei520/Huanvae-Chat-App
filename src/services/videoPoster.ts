 ...
 * 封面内容按 `file_hash` 定身份、落盘后不再变，同一会话里验第二次没有信息量；
...
/** 同一 fileHash 的在途截帧任务（多个格子同时首屏时只截一次）。 */
const inflight = new Map<string, Promise<string | null>>();

/**
 * 本会话已经验过「不是黑帧」的封面**落盘路径**。
 *
 * 键用 `local_path` 而不是 `file_hash`：路径里含 `data/{user}_{server}/`，
 * 换账号后同一个 `file_hash` 指向的是另一个人的文件，必须重新验一次。
 ...
 */
export async function loadVideoPosterSrc(fileHash: string): Promise<string | null> {
  if (!fileHash) {
    return null;
  }
  let localPath: string | null;
  try {
    localPath = await invoke<string | null>('get_video_poster_path', { fileHash });
  } catch (e) {
  ...
  if (pixels && isNearlyBlackFrame(pixels)) {
    console.warn('[videoPoster] 已落盘封面是黑帧，作废并重取:', fileHash);
    try {
      await invoke('invalidate_video_poster', { fileHash });
    } catch (e) {
 ...
 * 截一帧并落盘，返回落盘后的显示 src。
 *
 * 同一 fileHash 并发调用只会真正截一次（在途任务共享）；本会话内失败过的直接返回 null，
 * 理由见 `captureFailed` 的注释。
 *
 * @param fileHash 视频的稳定身份（同时是封面的键）
...
export function captureAndSaveVideoPoster(
  ...
  if (!fileHash || !videoSrc || captureFailed.has(fileHash)) {
    return Promise.resolve(null);
  }
  const pending = inflight.get(fileHash);
  if (pending) {
    return pending;
  }
  ...
  const task = (async (): Promise<string | null> => {
    ...
    try {
      ...
      if (isNearlyBlackFrame(frame.pixels)) {
        captureFailed.add(fileHash);
        console.warn('[videoPoster] 截到的是黑帧，拒绝写入封面缓存:', fileHash);
        return null;
      }
      ...
      captureFailed.add(fileHash);
      console.warn('[videoPoster] 截取/保存封面失败:', fileHash, e);
      return null;
    } finally {
      releaseSlot();
