 ...
 *
 * 置顶分层：comparePinnedThenTime 先按 isPinned 降序分层（置顶在前），
 * 同层内走 compareByTimeDesc（chat / group tab 用）；friends tab 外层是
 * 特别关心分层，内层仍用 compareByTimeDesc。
 */
...
/** 纯时间降序（新 → 旧），并列按 uniqueKey 稳定 tie-break（friends tab 内层等场景用） */
...
/** 置顶优先分层（isPinned 降序），同层内纯时间降序（chat / group tab 用） */
export function comparePinnedThenTime(a: SortableCard, b: SortableCard): number {
