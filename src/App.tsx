...
function App() {
  ...
  const handleSelectAccount = useCallback(async (account: SavedAccount) => {
    ...
    try {
      ...
      if (isMobile()) {
        ...
        // - macOS：直接解密 App 私有 AES 加密文件（无系统钥匙串弹框，v1.1.24 起也不再要 Touch ID）。
        ...
        try {
          ...
          setError(notSaved ? '密码未保存，请重新输入密码登录' : '读取保存的密码失败，请手动登录');
          return;
        }
      }
  ...
  const handleLogin = useCallback(async (
    ...
    try {
      ...
      //    无条件 saveAccount 写密码：macOS 写 App 私有 AES（静默、无弹框）、Windows 写凭据管理器
      //    （静默）、Linux 写 Secret Service。必须每次写——否则已存在账号迁移到新存储后首次手动
      //    登录不会把密码种进存储，会陷入"密码未保存"死循环。saveAccount 同时刷新昵称等元数据。
      await Promise.all([
