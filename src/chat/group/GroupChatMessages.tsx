        ...
        <LayoutGroup>
          <AnimatePresence mode="popLayout">
            {sortedMessages.map((message) => {
              ...
              // 与好友侧一致：合格消息（非发送中/失败/撤回）一律给回执位（无条件渲染），文案异步填充。
              // memberCount/positions 异步未到、或不适用（msgSeq<=0 / 单人群 eligible<=0）时为空串占位，
              // ReadReceiptLabel 用 minHeight 预留行高 → 回执行高度首帧即定、迟到的真实文案只换字不换高 → 零回流。
              let readReceipt: { text: string } | undefined;
              if (message.sendStatus !== 'sending' && message.sendStatus !== 'failed' && !message.is_recalled) {
                readReceipt = {
                  text: groupReadReceiptText(message.seq, countReaders(message.seq, message.sender_id), memberCount - 1) ?? '',
                };
              }
