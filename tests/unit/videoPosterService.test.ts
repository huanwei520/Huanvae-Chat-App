...
/**
 * 每个用例用**独立**的 posterKey。
 *
 * 模块内的「本会话失败过就不再重试」集合与「在途任务表」都按 posterKey 记，且**故意**没有
...
describe('loadVideoPosterSrc：只读本地索引，键是 posterKey', () => {
  it('命中：按 posterKey 查，落盘路径经同一条 asset 显示通道转成 src', async () => {
    const posterKey = nextHash('hit');
    core.invoke.mockResolvedValue('/data/u_s/file/posters/x.jpg');

    const src = await loadVideoPosterSrc(posterKey);

    expect(core.invoke).toHaveBeenCalledWith('get_video_poster_path', { fileKey: posterKey });
  ...
  it('空 posterKey 直接返回 null，不发 IPC', async () => {
...
describe('captureAndSaveVideoPoster：截一次 → 落盘 → 返回本地 src', () => {
  it('把截出来的字节交给 save_video_poster，键仍是 posterKey', async () => {
    const posterKey = nextHash('save');
    capture.captureVideoFrame.mockResolvedValue(litFrame([1, 2, 3]));
    core.invoke.mockResolvedValue('/data/u_s/file/posters/y.jpg');

    const src = await captureAndSaveVideoPoster(posterKey, 'http://127.0.0.1:1/v.mp4');
  ...
  it('同一 posterKey 并发调用只真截一次（多个格子同屏也只有一份工作）', async () => {
    const posterKey = nextHash('dedupe');
    const d = deferred<CapturedFrame>();
    capture.captureVideoFrame.mockReturnValue(d.promise);
    core.invoke.mockResolvedValue('/p/z.jpg');

    const a = captureAndSaveVideoPoster(posterKey, 'src://v');
    const b = captureAndSaveVideoPoster(posterKey, 'src://v');
  ...
  it('截帧失败 ⇒ 返回 null，且本会话内不再对同一个视频重试', async () => {
    const posterKey = nextHash('fail');
    capture.captureVideoFrame.mockRejectedValue(new Error('视频加载失败'));

    expect(await captureAndSaveVideoPoster(posterKey, 'src://v')).toBeNull();
    expect(capture.captureVideoFrame).toHaveBeenCalledTimes(1);
  ...
  it('落盘失败也算失败（返回 null，不把半成品当成功）', async () => {
    const posterKey = nextHash('savefail');
  ...
  it('缺 posterKey 或缺 src ⇒ 不截帧（没有键就没法索引，截了也存不住）', async () => {
...
describe('写入闸门：黑帧绝不落盘（缓存了黑帧 = 每次都错，且自己不会恢复）', () => {
  it('纯黑帧 ⇒ 不调 save_video_poster、返回 null', async () => {
    const posterKey = nextHash('black');
  ...
  it('黑帧走的是快速失败：本会话不再重试（下次启动才重来）', async () => {
    const posterKey = nextHash('blackretry');
  ...
  it('正对照：同一条路径上，有内容的帧照常落盘（证明闸门不是恒拒）', async () => {
    const posterKey = nextHash('blacklit');
    capture.captureVideoFrame.mockResolvedValue(litFrame([4, 5]));
    core.invoke.mockResolvedValue('/p/ok.jpg');

    expect(await captureAndSaveVideoPoster(posterKey, 'src://v')).toBe('asset://localhost//p/ok.jpg');
    expect(core.invoke).toHaveBeenCalledWith('save_video_poster', { fileKey: posterKey, bytes: [4, 5] });
  });
});

describe('自愈：已落盘的黑帧要能自己好，不能只堵写入口', () => {
  it('读到的封面是黑帧 ⇒ 作废该条目并返回 null（上层退回重截）', async () => {
    const posterKey = nextHash('healblack');
    ...
    expect(core.invoke).toHaveBeenCalledWith('invalidate_video_poster', { fileKey: posterKey });
  });

  it('封面正常 ⇒ 不作废、照常返回 src（证明自愈不是恒删）', async () => {
    const posterKey = nextHash('healok');
    core.invoke.mockImplementation(async (cmd: string) =>
    ...
    expect(await loadVideoPosterSrc(posterKey)).toBe('asset://localhost//data/u_s/file/posters/good.jpg');
    expect(core.invoke.mock.calls.filter((c) => c[0] === 'invalidate_video_poster')).toHaveLength(0);
  });

  it('读不出像素 ⇒ 保持现状，绝不删掉一张可能是好的封面', async () => {
    const posterKey = nextHash('healunreadable');
    core.invoke.mockImplementation(async (cmd: string) =>
      (cmd === 'get_video_poster_path' ? '/data/u_s/file/posters/unreadable.jpg' : null));
    capture.readImagePixels.mockResolvedValue(null);

    expect(await loadVideoPosterSrc(posterKey)).toBe('asset://localhost//data/u_s/file/posters/unreadable.jpg');
    expect(core.invoke.mock.calls.filter((c) => c[0] === 'invalidate_video_poster')).toHaveLength(0);
  });

  it('同一落盘路径本会话只解码验一次（自愈不该每次挂载都重解码一张图）', async () => {
    const posterKey = nextHash('healonce');
    core.invoke.mockImplementation(async (cmd: string) =>
