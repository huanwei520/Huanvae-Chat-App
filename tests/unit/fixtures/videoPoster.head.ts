/**
 * 【常驻取证 fixture · 勿删】修复前（HEAD）的视频封面服务源码副本
 *
 * 来源：`git show HEAD:src/services/videoPoster.ts`（commit HEAD，工作区修复前基线）
 * 与 HEAD 的全部差异只有本注释块 + 下面两行 import 的**路径改写**（逻辑逐字节未动）：
 *   - `from './assetUrl'`            → `from '../../../src/services/assetUrl'`
 *   - `from './videoPosterCapture'`  → `from '../../../src/services/videoPosterCapture'`
 * （本文件位于 tests/unit/fixtures/，相对路径不同，故仅改 import specifier 以便解析到
 *   同一批被测模块；`@tauri-apps/api/core` 与 videoPosterCapture 由测试文件的 vi.mock
 *   按解析后同一路径接管，新旧逻辑共用同一组计数器。）
 *
 * 用途：tests/unit/videoCoverCache.test.tsx 在测试内**直接调用旧逻辑**，
 *      取「修复前」计数与现链路对比（禁止临时文件删除式取证，故此副本常驻）。
 * 复核：`diff <(git show HEAD:src/services/videoPoster.ts) tests/unit/fixtures/videoPoster.head.ts`
 *      应只显示本注释块与两行 import 路径。
 */
...
import { localPathToDisplaySrc } from '../../../src/services/assetUrl';
import { captureVideoFrame, isNearlyBlackFrame, readImagePixels } from '../../../src/services/videoPosterCapture';
