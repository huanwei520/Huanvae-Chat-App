...
export const FEATURE_CHECKLIST: FeatureCategory[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        name: '卡片最新消息预览',
        description: '聊天卡片显示本地数据库中最新一条消息',
        critical: true,
        steps: [
          '发送一条文本消息给好友',
          '返回消息列表，验证该好友卡片显示刚发送的消息内容',
          '发送一张图片消息',
          '验证卡片预览更新为 "[图片]"',
          '撤回一条消息',
          '验证卡片预览自动更新为前一条消息',
      ...
      {
        name: 'Markdown 消息渲染',
        description: '聊天消息支持 Markdown 格式渲染（标题/列表/代码块/表格/引用/链接）',
        critical: true,
        steps: [
          '发送包含 Markdown 格式的消息（如 **加粗**、`代码`、列表）',
          '验证对方收到的消息正确渲染 Markdown 格式',
          '发送包含代码块的消息（带语言标识，如 ```js）',
          '验证代码块有语法高亮和复制按钮',
          '验证 AI 助手回复的 Markdown 内容正确渲染',
          '验证群聊消息同样支持 Markdown 渲染',
          '验证自己的消息（蓝色背景）代码高亮颜色适配',
        ],
      },
      {
        name: '发送文本消息',
  ...
  // ============== AI 助手模块 ==============
  {
    ...
    items: [
      {
        name: 'AI 卡片置顶显示',
        description: '消息列表顶部始终显示 AI 助手卡片，预览显示当前会话标题',
        critical: true,
        steps: [
          '登录后切换到消息标签',
          '验证列表第一项为 AI 助手卡片',
          '验证卡片显示 "AI" 头像和 "AI 助手" 名称',
          '验证即使有未读消息，AI 卡片仍在第一位',
          '无会话时预览显示"有什么可以帮你的？"',
          '发起对话后预览自动更新为当前会话标题',
          '切换会话后预览同步更新标题',
        ],
      ...
      {
        name: 'AI 消息发送',
        description: '发送消息并获取 AI 回复（SSE 流式）',
        critical: true,
        steps: [
          '在 AI 对话中输入消息',
          '点击发送',
      ...
      {
        name: 'AI 工具调用状态显示',
        description: 'AI 调用工具时实时显示执行状态',
        critical: true,
        steps: [
          '向 AI 发送需要工具调用的消息（如"我有哪些好友"）',
          '验证显示工具调用指示器（如"正在查询好友列表..."）',
          '验证工具执行完成后指示器变为完成状态',
          '验证 AI 继续输出最终回复内容',
          '验证回复完成后工具指示器消失',
        ],
      ...
      },
      {
        ...
        steps: [
          ...
          '验证列表包含标题、消息数、时间',
          '验证当前会话高亮显示',
          '点击某条历史会话，验证消息切换到对应会话',
          '验证抽屉自动关闭',
          'hover 会话项显示删除按钮',
          '点击删除按钮出现确认状态，再次点击确认删除',
          '删除当前会话时验证消息区域清空',
          '按 ESC 或点击遮罩关闭抽屉',
          '移动端验证同样的功能可用',
        ],
