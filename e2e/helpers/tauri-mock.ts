      ...
      window.__TAURI_INTERNALS__ = {
        invoke: async (cmd, args) => {
          ...
          const businessMocks = {
            ...
            // 🔴 通知音相关：返回值形状必须对。list_notification_sounds 若返回 null，
            //    <SoundSelector> 直接 .length ⇒ 抛错 ⇒ **整棵 React 树卸载**（页面全白）。
            //    这是本次新增的登录后 e2e 撞出来的 —— 在此之前没有任何一条 e2e 能走到设置面板。
            'ensure_sounds_directory': null,
